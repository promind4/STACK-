...
import {
  ...
  Cpu, HardDrive, Smartphone, Mic2, Monitor, Speaker, RefreshCw, Cable, Layers
} from 'lucide-react';
...
const QuestionTitle = ({ children, subtitle }: any) => (
  <div className="text-center mb-12 space-y-2">
    <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white font-serif">{children}</h1>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        {/* Image - Clickable */}
        <div
          onClick={() => onNavigate('product', item.selected.slug)}
        ...
        >
          <img
            ...
            className="w-full h-full object-contain mix-blend-multiply group-hover/img:scale-105 transition-transform"
        ...
        <div className="flex-1 relative z-10 flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <h3
            onClick={() => onNavigate('product', item.selected.slug)}
            className="text-xl font-bold font-serif text-zinc-900 mb-1 hover:text-primary cursor-pointer transition-colors"
          >
            {item.selected.name}
          </h3>
          {explanation && (
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center md:justify-start">
            ...
            {/* FIXED: Black text on white button for contrast */}
            <button
              onClick={() => {
              ...
              className="px-4 py-2 bg-primary text-white hover:bg-primary/90 rounded-lg text-sm font-bold transition-colors"
            >
              Voir l'offre
            </button>
            ...
            <div className="grid gap-3">
              {item.alternatives.map((alt, idx) => (
                <div key={alt.id} className="flex items-center gap-4 p-3 bg-zinc-50 rounded-xl border border-zinc-100">
                  <img src={alt.image_url} className="w-12 h-12 object-contain mix-blend-multiply" alt={alt.name} />
                  ...
                  <button
                    ...
                    className="px-3 py-1 bg-primary text-white hover:bg-primary/90 rounded text-xs"
                  >
                    Voir
                  </button>
                </div>
...
export const ConfiguratorPage: React.FC<ConfiguratorPageProps> = ({ onNavigate }) => {
  ...
  useSEO({
    title: 'Configurateur IA - Trouvez votre Setup Idéal',
    description: 'Notre configurateur intelligent analyse vos besoins et votre budget pour vous recommander le setup audio, vidéo ou streaming parfait. Résultat en 60 secondes.',
  });
  ...
  return (
    <motion.div
      ...
      className="min-h-screen bg-[#050505] text-white font-sans selection:bg-primary/30 relative overflow-x-hidden"
    >
      ...
      <div className="pt-32 pb-40 min-h-screen relative z-10 container mx-auto px-6 max-w-5xl">
        ...
        {isCalculating ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-8">
            ...
            <h2 className="text-2xl font-bold font-serif">Analyse en cours...</h2>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-4xl mx-auto space-y-10">
                ...
                {/* Music Specific Sub-Question */}
                {(ctx.usage === 'music_vocals' || ctx.usage === 'music_instruments') && (
                  <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6 text-center space-y-4">
                    <h3 className="text-zinc-400 uppercase tracking-widest text-xs font-bold">Votre Pratique</h3>
                    <div className="flex justify-center gap-4">
                      <button
                        onClick={() => updateCtx('usage', 'music_vocals')}
                        className={`px-6 py-3 rounded-xl border flex items-center gap-2 transition-all ${ctx.usage === 'music_vocals' ? 'bg-primary text-white border-primary' : 'bg-transparent text-zinc-500 border-zinc-700'}`}
                      >
                        <Mic2 size={16} /> Voix Uniquement
                      </button>
                      <button
                        onClick={() => updateCtx('usage', 'music_instruments')}
                        className={`px-6 py-3 rounded-xl border flex items-center gap-2 transition-all ${ctx.usage === 'music_instruments' ? 'bg-primary text-white border-primary' : 'bg-transparent text-zinc-500 border-zinc-700'}`}
                      >
                        <Music size={16} /> Instruments & Voix
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[{ id: 'hasMic', label: 'Microphone' }, { id: 'hasInterface', label: 'Interface' }, { id: 'hasCamera', label: 'Caméra' }, { id: 'hasLights', label: 'Éclairage' }].map((item) => (
                      <div key={item.id} className="flex flex-col gap-2">
                        ...
                        {/* NEW: Sub-option for Mic */}
                        {item.id === 'hasMic' && ctx.hasMic && (
                          <div className="flex gap-2 pl-4">
                            <span className="text-xs text-zinc-500 self-center">Type:</span>
                            <button
                              onClick={() => updateCtx('existingMicType', 'usb')}
                              className={`flex-1 py-1.5 text-xs rounded-lg border transition-all ${ctx.existingMicType === 'usb' ? 'bg-white text-black border-white' : 'bg-zinc-900 text-zinc-500 border-zinc-800 hover:border-zinc-600'}`}
                            >
                              USB
                            </button>
                            <button
                              onClick={() => updateCtx('existingMicType', 'xlr')}
                              className={`flex-1 py-1.5 text-xs rounded-lg border transition-all ${ctx.existingMicType === 'xlr' ? 'bg-white text-black border-white' : 'bg-zinc-900 text-zinc-500 border-zinc-800 hover:border-zinc-600'}`}
                            >
                              XLR
                            </button>
                          </div>
            ...
            {step === 5 && result && (
              <motion.div key="s5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-6xl mx-auto space-y-10">
                <div className="text-center space-y-3">
                  ...
                  <h1 className="text-3xl md:text-5xl font-black font-serif text-white">
                    Votre Setup <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-400">Complet</span>
                  </h1>
                  <p className="text-zinc-500 text-sm">
                    {/* DEBUG: Show if result is 0 */}
                    {result.totalCost === 0 && (
                      <div className="p-4 bg-red-500/20 border border-red-500 rounded text-red-200 text-xs mb-4">
                        <strong className="block text-lg mb-2">⚠️ Oups, aucun résultat trouvé.</strong>
                        <div className="text-left font-mono">
                          <p>Produits chargés: {products.length}</p>
                          {error && <p className="text-yellow-400">Erreur: {error}</p>}
                          <p>Env Var URL: {import.meta.env.VITE_SUPABASE_URL ? 'PRESENT' : 'MISSING'}</p>
                          <p>Budget: {ctx.budget}€</p>
                          <p>Usage: {ctx.usage}</p>
                          <p>Room: {ctx.room}</p>
                          <p>Vibe: {ctx.vibe}</p>
                          <p>Computer: {ctx.computer}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {(() => {
                      ...
                      const pct = Math.round((visibleTotal / ctx.budget) * 100);
                      return <>Budget utilisé: <span className="text-white font-mono">{pct}%</span> ({visibleTotal}€ / {ctx.budget}€)</>;
                    })()}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div className="lg:col-span-8 space-y-4">
                    ...
                    {result.lights.length > 0 && result.lights.map((light) => (
                      <div key={light.selected.id}>
                        <ProductCard
                          title="Éclairage"
                          item={light}
                    ...
                    {/* Accessories Section — HIDDEN: set to true to re-enable */}
                    {false && result.accessories.length > 0 && (
                      <div className="bg-zinc-900/30 border border-white/5 rounded-2xl p-5">
                        <h3 className="text-sm font-bold uppercase text-zinc-400 mb-4 flex items-center gap-2">
                          <Cable size={16} /> Accessoires Essentiels
                        </h3>
                        <div className="grid gap-3">
                          {result.accessories.map((acc) => (
                            <div key={acc.selected.id} className="flex items-center gap-4 p-3 bg-zinc-800/50 rounded-xl group/acc">
                              <img
                                onClick={() => onNavigate('product', acc.selected.slug)}
                                src={acc.selected.image_url}
                                className="w-14 h-14 object-contain cursor-pointer hover:scale-105 transition-transform"
                                alt={acc.selected.name}
                              />
                              <div className="flex-1">
                                <p
                                  onClick={() => onNavigate('product', acc.selected.slug)}
                                ...
                                <p className="text-xs text-zinc-500">{result.explanations[acc.selected.id]}</p>
                              </div>
                              <span className="text-sm font-mono text-white">≈ {acc.selected.price}€</span>
                              <button
                                onClick={() => {
                                  const bestLink = acc.selected.offers && acc.selected.offers.length > 0
                                    ? [...acc.selected.offers].sort((a, b) => a.price - b.price)[0].affiliate_link
                                    : '#';
                                  openLink(bestLink || '#');
                                }}
                                className="px-3 py-1 bg-zinc-700 text-white hover:bg-zinc-600 rounded text-xs"
                              >
                                Voir
                              </button>
                            </div>
                  ...
                  <div className="lg:col-span-4">
                    <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 sticky top-24 space-y-5">
                      <h3 className="text-lg font-bold font-serif flex items-center gap-2 text-white">
                      ...
                      <div className="space-y-3">
                        ...
                        {/* Accessories recap — HIDDEN: uncomment to re-enable
                        {result.accessories.map(acc => (
                          <div key={acc.selected.id} className="flex justify-between text-sm text-zinc-400"><span>{acc.selected.name.slice(0, 20)}...</span><span>≈ {acc.selected.price}€</span></div>
                        ))}
                        */}
                      ...
                      <p className="text-xs text-zinc-500 text-center italic">Cliquez sur "Voir l'offre" sur chaque produit ci-dessus.</p>

                      <div className="text-center">
                        <p className="text-[10px] text-zinc-600">Score de correspondance: {result.matchScore}%</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {step < 5 && !isCalculating && (
          <div className="fixed bottom-0 left-0 w-full p-5 bg-black/90 backdrop-blur-xl border-t border-white/10 z-50">
            <div className="max-w-4xl mx-auto flex justify-between items-center">
              <Button variant="ghost" className="text-zinc-400 hover:text-white" onClick={() => setStep(Math.max(1, step - 1))} disabled={step === 1}>
                <ChevronLeft className="mr-2 h-4 w-4" /> Retour
              </Button>
              <Button className="bg-primary hover:bg-primary/90 text-white px-8 py-3 rounded-xl font-bold" onClick={handleNext}>
