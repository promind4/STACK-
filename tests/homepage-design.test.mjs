...
const trustStrip = await readFile(new URL('../components/home/HomeTrustStrip.tsx', import.meta.url), 'utf8').catch(() => '')
...
assert.equal(compatibility.includes('0{index + 1}'), false, 'the compatibility graph must not use dynamic numeric labels')
assert.equal(contextGrid.includes('num:'), false, 'the hero context grid must not retain repeated numeric labels')
assert.match(trustStrip, /aria-hidden/, 'the illustrated trust strip must mark decorative artwork as hidden')
assert.equal(trustStrip.toLowerCase().includes('livraison offerte'), false, 'the trust strip must not claim free delivery')
